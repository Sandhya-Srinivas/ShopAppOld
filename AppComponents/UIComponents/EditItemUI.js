      ...
      <Card style={styles.card}>
        <Form>
          <Item floatingLabel>
            ...
            <Input disabled value={itemName} onChangeText={(inputText)=>setItemName(inputText)}/>
          </Item>
          <Item floatingLabel>
            <Label><Text>Quantity</Text></Label>
            <Input keyboardType={'number-pad'} value={quantity} onChangeText={(inputText)=>callbackSetQuan(inputText)}/>
          </Item>
          ...
          <Item style={{width: "95%", alignSelf: 'center'}}> 
            <Left> 
              ...
              {unitFlag? 
                <Input keyboardType={'number-pad'} value={cost} onChangeText={(inputText)=>{callbackSetCost(inputText); setCostType(k.unitCost)}}/> 
                : <Input onFocus={()=>{setTotalFlag(false); setUnitFlag(!unitFlag); setCostType(k.unitCost)}} />
              }              
              </Item>
            </Left>
            <Right>   
              ...
              {totalFlag? 
              <Input keyboardType={'number-pad'} value={cost} onChangeText={(inputText)=>{callbackSetCost(inputText); setCostType(k.totalCost)}}/>
              : <Input onFocus={()=>{setUnitFlag(false); setTotalFlag(!totalFlag); setCostType(k.totalCost);}} />
              } 
              </Item>
            </Right>
          </Item>

          <Label style={{alignSelf: 'center'}}><Text>Choose expiry</Text></Label> 
          <Item style={{alignSelf: 'center'}}>
          <Left>  
              <Item floatingLabel>
                <Label>In Days</Label>                
                {expiryInDays ? 
                <Input keyboardType={'number-pad'} disabled={noExpiry} value={expiryDays} onChangeText={(days)=>{handleDaychange(days);}} />
                  :<Input disabled={noExpiry} onFocus={()=>{setExpiryAsDate(false); setExpiryInDays(true)}} />           
                }
              </Item>
            </Left>
            <Body >  
              <Item floatingLabel>
                ...
                {expiryAsDate? 
                <Input disabled={noExpiry} onPressIn={()=>{setShowDP(true);}} onChangeText={()=>setShowDP(true)} value={moment(expiryDate).format('DD/MM/YYYY')} />
                  :<Input disabled={noExpiry}  onFocus={()=>{setExpiryAsDate(true); setExpiryInDays(false); setShowDP(true) }} />           
                }
                </Item>
            </Body>
